...
|----|------|---|---|------------|
| 🔴 R-1 | **Motra ToS §8 prohibits automated/systematic retrieval & DB compilation without written permission; app use is "personal, non-commercial" unless approved. Path B proceeds without permission = ToS breach risk (access cutoff / cease-and-desist).** Eli has accepted this risk. | M | H | Send the drafted permission email (converts to permission-pending); keep Motra-agnostic data spine + manual/Apple Health fallback so the app survives a cutoff; original visualizations only; tokens encrypted, user-revocable. Revisit if Motra responds. |
